...
// 🔁 Custom hook that fetches ML + GPT2-based advice from FastAPI
const useMLModel = (battery, weatherData, deviceState) => {
  const [advice, setAdvice] = useState("");
  const [impact, setImpact] = useState(null);
  const [alertLevel, setAlertLevel] = useState("");
  const [optionalAction, setOptionalAction] = useState("");
  ...
  useEffect(() => {
    const fetchBatteryAdvice = async () => {
      try {
        ...
        const data = response.data;
        setAdvice(data.natural_language_tip);
        setImpact(data.predicted_health_impact);
        setAlertLevel(data.alert_level);
        setOptionalAction(data.optional_action);
      } catch (error) {
        console.error("Failed to fetch AI advisory:", error);
      }
    };

    if (battery && weatherData && deviceState) {
      fetchBatteryAdvice();
    }
  }, [battery, weatherData, deviceState]);

  return { advice, impact, alertLevel, optionalAction };
};
