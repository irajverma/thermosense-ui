...
  // MODIFIED to accept thresholds from settings
  const generateRecommendations = useCallback((batteryData, weatherData, deviceTemp, performanceData, thresholds) => {
    const recommendations = [];
    let healthScore = 100;
    let alertLevel = 'safe';

    // Temperature analysis using thresholds
    if (deviceTemp > thresholds.tempCritical) {
      recommendations.push("🚨 CRITICAL: Device temperature too high! Cool down immediately!");
      healthScore -= 25;
      alertLevel = 'danger';
    } else if (deviceTemp > thresholds.tempWarning) {
...
    // Battery analysis
    if (batteryData) {
      if (batteryData.charging && deviceTemp > thresholds.tempWarning) {
...
      }
      
      if (batteryData.level < thresholds.batteryLow) {
...
  const { batteryData, weatherData, deviceTemp, settings } = useContext(ThermoSenseContext); // Get settings from context
...
        return newData;
      });
    }, settings.chartInterval); // USE SETTING for interval

    return () => clearInterval(interval);
  }, [isPaused, deviceTemp, weatherData, batteryData, settings.chartInterval]); // Add settings to dependency array
...
// --- CORRECTED Settings Component ---
// This version is now a "controlled component" that gets its state and handlers from its parent (ThermoSenseApp)
const Settings = ({ isVisible, settings, onSettingChange, onReset }) => {
  if (!isVisible) return null;

  // This handler now calls the function passed via props
    ...
    <div>
      ...
      <div className="settings-grid">
        <div className="settings-section card">
          ...
          <div className="form-group">
            <label className="form-label">Device Temp. Sim Interval</label>
            <select 
              className="form-control" 
...
// This version now holds the settings state and provides it to all children via context.
...
  }, []);
  ...
  const handleResetSettings = useCallback(() => {
    ...
    addNotification({ type: 'info', message: '⚙️ Settings have been reset to default.' });
  }, [addNotification]);
  // --- END LIFTED STATE ---
...
  // Health data calculation (now uses settings)
  const healthData = useMemo(() => {
    return generateRecommendations(batteryData, weatherData, deviceTemp, performanceData, settings);
  }, [batteryData, weatherData, deviceTemp, performanceData, settings, generateRecommendations]);
...
        setLoadingSteps(prev => prev.map((step, index) => index === i ? { ...step, status: 'active' } : step));
        await new Promise(resolve => setTimeout(resolve, steps[i].duration));
        setLoadingSteps(prev => prev.map((step, index) => index === i ? { ...step, status: 'completed' } : step));
...
  // Device temperature simulation (now uses settings)
  useEffect(() => {
    const interval = setInterval(() => {
      let baseTemp = 25;
...
    }, settings.batteryInterval); // Use setting
    return () => clearInterval(interval);
  }, [weatherData, batteryData, performanceData, settings.batteryInterval]);

  // Periodic weather updates (now uses settings)
  useEffect(() => {
    const weatherIntervalId = setInterval(() => {
        if (weatherStatus === 'connected') {
            retryWeather();
        }
    }, settings.weatherInterval); // Use setting
...
    } catch (error) {
      addNotification({ type: 'warning', message: '⚠️ Some permissions could not be enabled - running in limited mode' });
    }
  };
...
    addNotification({ type: 'info', message: '⚠️ Running in limited mode - Enable permissions for full functionality' });
  };
...
    const data = { timestamp: new Date().toISOString(), batteryData, weatherData, performanceData, deviceTemp, healthData, notifications, apiStatus };
...
    addNotification({ type: 'info', message: '🧪 Test notification - System is working correctly!' });
  }, [addNotification]);

  // Context value (now includes settings)
  const contextValue = {
    batteryData,
    weatherData,
    performanceData,
...
    healthData,
    notifications,
    settings, // Provide settings to all consumers
...
        <PermissionModal isVisible={showPermissionModal} onAllow={handleAllowPermissions} onDeny={handleDenyPermissions} />
